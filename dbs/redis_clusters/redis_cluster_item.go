...
// connection
func (item *ItemInfo) GetRedisItemFromConf(c *config.ConfigEngine, name string) {
	r := new(RedisItemYaml)
	ret := c.GetStruct(name, r).(*RedisItemYaml)
	item.prefix = ret.Prefix
	item.size = int64(ret.Len)
	item.expire = time.Duration(ret.Expire) * time.Second
	errs.CheckEmptyValue(item.prefix)
}
